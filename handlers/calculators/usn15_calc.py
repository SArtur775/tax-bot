...
async def start_usn15_calculator(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"📊 <b>Калькулятор УСН 15% (Доходы-Расходы)</b>\n\n"
        f"{limit_text}\n\n"
...
@usn15_router.callback_query(F.data == "new_usn15")
async def new_usn15_calculation(callback: CallbackQuery, state: FSMContext):
    await start_usn15_calculator(callback.message, state)
    await callback.answer()

@usn15_router.callback_query(F.data == "save_to_history")
async def save_to_history(callback: CallbackQuery):
    await callback.answer("✅ Расчет уже сохранен в вашу историю!", show_alert=True)

@usn15_router.callback_query(F.data == "main_menu")
